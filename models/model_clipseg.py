...
class CLIPSegImageSegmentationOutput(ModelOutput):
    ...
    def to_tuple(self) -> Tuple[Any]:
        return tuple(
            self[k] if k not in ["vision_model_output",
                                 "decoder_output"] else getattr(self, k).to_tuple()
            for k in self.keys()
        )
    ...
    def interpolate_position_embeddings(self, new_size):
        ...
        b = (
            nn.functional.interpolate(
                a, new_size, mode="bicubic", align_corners=False)
            .squeeze(0)
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        if embeddings.shape[1] != self.num_positions:
            ...
            embeddings = embeddings + \
                self.interpolate_position_embeddings((new_shape, new_shape))
...
class CLIPSegEncoder(nn.Module):
    ...
    def __init__(self, config: CLIPSegConfig):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([CLIPSegEncoderLayer(
...
class CLIPSegDecoder(CLIPSegPreTrainedModel):
    def __init__(self, config: CLIPSegConfig):
        ...
        if config.use_complex_transposed_convolution:
            transposed_kernels = (
                config.vision_config.patch_size // 4, config.vision_config.patch_size // 4)

            self.transposed_convolution = nn.Sequential(
                nn.Conv2d(config.reduce_dim, config.reduce_dim,
        ...
        self.layers = nn.ModuleList([CLIPSegDecoderLayer(
            decoder_config) for _ in range(len(config.extract_layers))])
...
class CLIPSegForImageSegmentation(CLIPSegPreTrainedModel):
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            hidden_states = vision_outputs.hidden_states if return_dict else vision_outputs[
                2]
            ...
            if return_dict:
                ...
                vision_outputs = (
                    vision_outputs[:2] +
                    vision_outputs[3:] if not output_hidden_states else vision_outputs
                )
        ...
        if not return_dict:
            output = (logits, conditional_embeddings, pooled_output,
                      vision_outputs, decoder_outputs)
            return ((loss,) + output) if loss is not None else output
