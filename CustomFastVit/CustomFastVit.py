...
__all__ = ['CustomFastVit']

def calculate_drop_path_rates(drop_path_rate, layer_depths, stagewise=False):
    """
    计算每个层的 drop path rates
    
    Args:
        drop_path_rate: 总的 drop path rate
        layer_depths: 每个阶段的层数
        stagewise: 是否按阶段分配 drop path rate
    
    Returns:
        每个阶段的 drop path rates 列表
    """
    num_stages = len(layer_depths)
    if drop_path_rate <= 0 or num_stages == 0:
        return [0.] * num_stages if stagewise else [[0.] * d for d in layer_depths]
    
    if stagewise:
        # 每个阶段使用相同的 drop path rate
        return [drop_path_rate] * num_stages
    else:
        # 根据深度线性分配 drop path rate
        total_blocks = sum(layer_depths)
        if total_blocks <= 1:
            return [[0.] * d for d in layer_depths]
        ...
        for depth in layer_depths:
            ...
            for _ in range(depth):
                stage_rates.append(drop_path_rate * block_id / (total_blocks - 1))
                block_id += 1
...
class MobileOneBlock(nn.Module):
    ...
    def __init__(
        ...
        self.se = SqueezeExcite(out_chs, rd_divisor=1,
                                **dd) if use_se else nn.Identity()
...
class ReparamLargeKernelConv(nn.Module):
    ...
    def __init__(
        ...
        self.se = SqueezeExcite(out_chs, rd_ratio=0.25,
                                **dd) if use_se else nn.Identity()
    ...
    def __init__(
        ...
        self.proj = nn.Sequential(
            ReparamLargeKernelConv(
                ...
                small_kernel=3,
                use_se=use_se,
                # NOTE original weights didn't use this act
...
class LayerScale2d(nn.Module):
    def __init__(
        ...
        self.gamma = nn.Parameter(
            init_values * torch.ones(dim, 1, 1, device=device, dtype=dtype))
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        if layer_scale_init_value is not None:
            self.layer_scale_1 = LayerScale2d(
                dim, layer_scale_init_value, **dd)
        else:
        ...
        if layer_scale_init_value is not None:
            self.layer_scale_2 = LayerScale2d(
                dim, layer_scale_init_value, **dd)
        else:
    ...
    def forward(self, x):
        x = x + \
            self.drop_path1(self.layer_scale_1(self.token_mixer(self.norm(x))))
    ...
    def __init__(
        ...
        if pos_emb_layer is not None:
            self.pos_emb = pos_emb_layer(
                dim_out, inference_mode=inference_mode, **dd)
        else:
        ...
        for block_idx in range(depth):
            if token_mixer_type == "repmixer":
                ...
                raise ValueError(
                    "Token mixer type: {} not supported".format(
                        token_mixer_type)
                )
...
class CustomFastVit(nn.Module):
    ...
    def __init__(
            ...
            token_mixers: Tuple[str, ...] = (
                "repmixer", "repmixer", "repmixer", "repmixer"),
        ...
        for i in range(len(layers)):
            ...
            if downsample:
                scale *= 2
            self.feature_info += [dict(num_chs=prev_dim,
                                       reduction=4 * scale, module=f'stages.{i}')]
    ...
    def forward_features(self, x: torch.Tensor, output_cls_token: bool = False) -> torch.Tensor:
        # input embedding
        x = self.stem(x)
        outs = []
        ...
        if self.fork_feat:
            ...
            if output_cls_token:
                # 如果需要cls_token且是fork_feat模式，计算cls_token并返回
                final_feature = self.final_conv(x)
                cls_token = self.forward_head(final_feature, pre_logits=False)
                batch, channels, _, _ = final_feature.shape
                final_feature = self.head.fc(final_feature.view(
                    batch, channels, -1).permute(0, 2, 1))
                return [cls_token, outs, final_feature]
            else:
                return outs
        x = self.final_conv(x)
        return x
    ...
    def forward(self, x: torch.Tensor, output_cls_token: bool = False) -> torch.Tensor:
        x = self.forward_features(x, output_cls_token)
        if self.fork_feat:
            if output_cls_token:
                # x 是 [cls_token, outs] 的形式
                return x
            else:
                # x 是 outs 的形式
                return x
...
default_cfgs = generate_default_cfgs({
    "custom_fastvit_t8.apple_in1k": _cfg(
        hf_hub_id='timm/'),
    "custom_fastvit_t12.apple_in1k": _cfg(
        hf_hub_id='timm/'),

    "custom_fastvit_s12.apple_in1k": _cfg(
        hf_hub_id='timm/'),
    "custom_fastvit_sa12.apple_in1k": _cfg(
        hf_hub_id='timm/'),
    "custom_fastvit_sa24.apple_in1k": _cfg(
        hf_hub_id='timm/'),
    "custom_fastvit_sa36.apple_in1k": _cfg(
        hf_hub_id='timm/'),

    "custom_fastvit_ma36.apple_in1k": _cfg(
        hf_hub_id='timm/',
        crop_pct=0.95),

    "custom_fastvit_t8.apple_dist_in1k": _cfg(
        hf_hub_id='timm/'),
    "custom_fastvit_t12.apple_dist_in1k": _cfg(
        hf_hub_id='timm/'),

    "custom_fastvit_s12.apple_dist_in1k": _cfg(
        hf_hub_id='timm/',),
    "custom_fastvit_sa12.apple_dist_in1k": _cfg(
        hf_hub_id='timm/',),
    "custom_fastvit_sa24.apple_dist_in1k": _cfg(
        hf_hub_id='timm/',),
    "custom_fastvit_sa36.apple_dist_in1k": _cfg(
        hf_hub_id='timm/',),

    "custom_fastvit_ma36.apple_dist_in1k": _cfg(
        hf_hub_id='timm/',
        crop_pct=0.95
    ),

    "custom_fastvit_mci0.apple_mclip": _cfg(
    ...
    "custom_fastvit_mci1.apple_mclip": _cfg(
    ...
    "custom_fastvit_mci2.apple_mclip": _cfg(
    ...
    "custom_fastvit_mci0.apple_mclip2_dfndr2b": _cfg(
        hf_hub_id='timm/',
        crop_pct=1.0,
        num_classes=512,  # CLIP proj dim
    ...
    "custom_fastvit_mci2.apple_mclip2_dfndr2b": _cfg(
        hf_hub_id='timm/',
        crop_pct=0.95,
        num_classes=512,  # CLIP proj dim
        mean=(0., 0., 0.), std=(1., 1., 1.),
    ...
    "custom_fastvit_mci3.apple_mclip2_dfndr2b": _cfg(
        hf_hub_id='timm/',
        crop_pct=0.95,
    ...
    "custom_fastvit_mci4.apple_mclip2_dfndr2b": _cfg(
        hf_hub_id='timm/',
        crop_pct=0.95,
        num_classes=768,  # CLIP proj dim
        mean=OPENAI_CLIP_MEAN, std=OPENAI_CLIP_STD,
...
def checkpoint_filter_fn(state_dict, model):
    ...
    for k, v in state_dict.items():
        ...
        # to match byobnet, regnet, nfnet
        k = k.replace('conv_exp', 'final_conv')
        ...
        if stage_idx is not None:
            ...
            if net_prefix + '.proj' in k:
                k = k.replace(net_prefix + '.proj',
                              stage_prefix + '.downsample.proj')
            elif net_prefix + '.pe' in k:
                k = k.replace(net_prefix + '.pe',
                              stage_prefix + '.pos_emb.pos_enc')
            else:
...
#     # 处理 fork_feat 模式下的权重兼容性
#     if hasattr(model, 'fork_feat') and model.fork_feat:
#         # fork_feat 模式：移除分类头相关权重
#         filtered_dict = {}
#         for k, v in out_dict.items():
#             # 过滤掉 final_conv 和 head 相关的权重
#             if not any(pattern in k for pattern in ['final_conv', 'head.']):
#                 filtered_dict[k] = v
#         return filtered_dict
#     elif hasattr(model, 'fork_feat') and not model.fork_feat:
#         # 分类模式：移除 fork_feat 相关权重（norm 层）
#         filtered_dict = {}
#         for k, v in out_dict.items():
#             if not re.match(r'norm[0-3]\.', k):
#                 filtered_dict[k] = v
#         return filtered_dict
...
def _create_custom_fastvit(variant, pretrained=False, **kwargs):
...
@register_model
def custom_fastvit_t8(pretrained=False, **kwargs):
    ...
    return _create_custom_fastvit('custom_fastvit_t8', pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def custom_fastvit_t12(pretrained=False, **kwargs):
    ...
    return _create_custom_fastvit('custom_fastvit_t12', pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def custom_fastvit_s12(pretrained=False, **kwargs):
    ...
    return _create_custom_fastvit('custom_fastvit_s12', pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def custom_fastvit_sa12(pretrained=False, **kwargs):
    """Instantiate FastViT-SA12 model variant."""
    model_args = dict(
        layers=(2, 2, 6, 2),
        embed_dims=(64, 128, 256, 512),
        mlp_ratios=(4, 4, 4, 4),
        pos_embs=(None, None, None, partial(
            RepConditionalPosEnc, spatial_shape=(7, 7))),
        token_mixers=("repmixer", "repmixer", "repmixer", "attention"),
    )
    return _create_custom_fastvit('custom_fastvit_sa12', pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def custom_fastvit_sa24(pretrained=False, **kwargs):
    ...
    return _create_custom_fastvit('custom_fastvit_sa24', pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def custom_fastvit_sa36(pretrained=False, **kwargs):
    ...
    return _create_custom_fastvit('custom_fastvit_sa36', pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def custom_fastvit_ma36(pretrained=False, **kwargs):
    ...
    return _create_custom_fastvit('custom_fastvit_ma36', pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def custom_fastvit_mci0(pretrained=False, **kwargs):
    ...
    return _create_custom_fastvit('custom_fastvit_mci0', pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def custom_fastvit_mci1(pretrained=False, **kwargs):
    ...
    return _create_custom_fastvit('custom_fastvit_mci1', pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def custom_fastvit_mci2(pretrained=False, **kwargs):
    ...
    return _create_custom_fastvit('custom_fastvit_mci2', pretrained=pretrained, **dict(model_args, **kwargs))


@register_model
def custom_fastvit_mci3(pretrained=False, **kwargs):
    ...
    model_args = dict(
        ...
        token_mixers=("repmixer", "repmixer", "repmixer",
                      "attention", "attention"),
        lkc_use_act=True,
        norm_layer=partial(LayerNorm2d, eps=1e-5),
        stem_use_scale_branch=False,
    )
    model = _create_custom_fastvit(
        'custom_fastvit_mci3', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def custom_fastvit_mci4(pretrained=False, **kwargs):
    ...
    model = _create_custom_fastvit(
        'custom_fastvit_mci4', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
